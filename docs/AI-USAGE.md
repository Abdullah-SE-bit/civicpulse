...
| 2026-09-26 | Claude Code / BETA | backend/app/config.py, backend test, post-merge review comments on #10-era PRs (#16, #20, #50, #74), evidence-ctx replication | Replaced localhost fallbacks with service names (updated the one test that asserted the old default), replicated the Docker context measurement, wrote review comments from what I ran | _pending review_ |
