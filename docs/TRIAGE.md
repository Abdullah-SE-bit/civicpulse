...
1. **Cache**: key is `sha256` of the whitespace/case-normalised text and location (`content_key`); value is the provider name plus the result JSON, TTL 24 h (`CACHE_TTL_SECONDS`). A hit skips the provider entirely. Fallback results are never cached.
