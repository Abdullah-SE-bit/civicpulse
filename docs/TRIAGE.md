...
1. **Cache**: key is `sha256` of the provider name plus the whitespace/case-normalised text and location (`content_key`), so switching provider never serves the old provider's results; value is the provider name plus the result JSON, TTL 24 h (`CACHE_TTL_SECONDS`). A hit skips the provider entirely. Fallback results are never cached.
